...
extern double ncephes_ncephes_cabs ( cmplx * );
...
#else
double fabs(), ncephes_ncephes_cabs(), sqrt(), atan2(), cos(), sin();
 ...
 * SYNOPSIS:
 *
 * double ncephes_ncephes_cabs();
 * cmplx z;
 * double a;
 *
 * a = ncephes_ncephes_cabs( &z );
 *
 *
 *
 * DESCRIPTION:
...
double 
ncephes_ncephes_cabs (register cmplx *z)
{
double x, y, b, re, im;
int ex, ey, e;

#ifdef INFINITIES
/* Note, ncephes_ncephes_cabs(INFINITY,NAN) = INFINITY. */
...
if( ey > MAXEXP )
	{
	mtherr( "ncephes_ncephes_cabs", OVERFLOW );
	return( INFINITY );
	}
if( ey < MINEXP )
	return(0.0);
 ...
 * SYNOPSIS:
 *
 * void ncephes_ncephes_csqrt();
 * cmplx z, w;
 *
 * ncephes_ncephes_csqrt( &z, &w );
 *
 *
 *
 * DESCRIPTION:
 ...
 *
 *                        2
 * Also tested by ncephes_ncephes_csqrt( z ) = z, and tested by arguments
 * close to the real axis.
 */


void 
ncephes_ncephes_csqrt (cmplx *z, cmplx *w)
{
cmplx q, s;
double x, y, r, t;
